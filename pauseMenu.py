...
def p():
    ...
    move_stats_4= {'bp':main.hero.move4bp,'acc': main.hero.move4acc, 'pp': main.hero.move4pp, 'efc': main.hero.move4effect, 'pri': main.hero.move4priority}
    ...
    def pause():
        ...
        stat_render_rect = stat_render.get_rect(center=(WIDTH/2-350,HEIGHT/2+110))
        screen.blit(stat_render, stat_render_rect)

        stat_render_2 = f.render(stat_text_2, True, (0, 0, 0))
        stat_render_rect_2 = stat_render_2.get_rect(center=(WIDTH/2-350,HEIGHT/2+130))
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                running = False
                    ...
                    if paused:
                        ...
                        m_stats_1 = f"Base Power: {move_stats_1['bp']}  Accuracy: {move_stats_1['acc']} Power Points: {move_stats_1['pp']} Effective: {move_stats_1['efc']} Priority: {move_stats_1['pri']}"
                        f=pygame.font.Font('kvn-pokemon-gen-5.ttf',20)
                        m_render_1 = f.render(m_stats_1, True, (0, 0, 0))
                        m_render_rect_1 = m_render_1.get_rect(center=(WIDTH/2,HEIGHT/2+210))

                        m_stats_2 = f"Base Power: {move_stats_2['bp']}  Accuracy: {move_stats_2['acc']} Power Points: {move_stats_2['pp']} Effective: {move_stats_2['efc']} Priority: {move_stats_2['pri']}"
                        m_render_2 = f.render(m_stats_2, True, (0, 0, 0))
                        m_render_rect_2 = m_render_2.get_rect(center=(WIDTH/2,HEIGHT/2+210))

                        m_stats_3 = f"Base Power: {move_stats_3['bp']}  Accuracy: {move_stats_3['acc']} Power Points: {move_stats_3['pp']} Effective: {move_stats_3['efc']} Priority: {move_stats_3['pri']}"
                        m_render_3 = f.render(m_stats_3, True, (0, 0, 0))
                        m_render_rect_3 = m_render_3.get_rect(center=(WIDTH/2,HEIGHT/2+210))

                        m_stats_4 = f"Base Power: {move_stats_4['bp']}  Accuracy: {move_stats_4['acc']} Power Points: {move_stats_4['pp']} Effective: {move_stats_4['efc']} Priority: {move_stats_4['pri']}"
                        m_render_4 = f.render(m_stats_4, True, (0, 0, 0))
                        m_render_rect_4 = m_render_4.get_rect(center=(WIDTH/2,HEIGHT/2+210))
        ...
        if move1==True:
            descBox = pygame.Rect(WIDTH/2-330, HEIGHT/2+195, 660, 60)
            pygame.draw.rect(screen, (255, 255, 255), descBox)
            pygame.draw.rect(screen, (0, 0, 0), descBox, 2)
